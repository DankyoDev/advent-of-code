...
fn solve(nums: &[i128], target: i128, allow_concat: bool) -> bool {
    ...
    for &n in &nums[1..] {
        let mut next: FxHashSet<i128> = FxHashSet::with_capacity_and_hasher(
            cur.len() * if allow_concat { 3 } else { 2 },
            Default::default(),
        );

        for &v in cur.iter() {
            if let Some(x) = v.checked_add(n) {
                if !positive || x <= target {
                    next.insert(x);
                }
            }
            if let Some(x) = v.checked_mul(n) {
                if !positive || x <= target {
                    next.insert(x);
                }
            }
